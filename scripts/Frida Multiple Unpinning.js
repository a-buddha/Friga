...
setTimeout(function() {
	Java.perform(function() {
		...
		// GMS TrustManagerImpl (Android > 7) //
		////////////////////////////////////////
		try {
			// Bypass GMS TrustManagerImpl.checkTrustedRecursive (Android > 7) {1}
			var array_list = Java.use("java.util.ArrayList");
			var GMS_TrustManagerImpl_Activity_1 = Java.use('com.google.android.gms.org.conscrypt.TrustManagerImpl');
			GMS_TrustManagerImpl_Activity_1.checkTrustedRecursive.implementation = function(certs, ocspData, tlsSctData, host, clientAuth, untrustedChain, trustAnchorChain, used) {
				console.log('[+] Bypassing GMS TrustManagerImpl (Android > 7) checkTrustedRecursive {1} check: '+ host + ' ' + trustAnchorChain + ' '+ untrustedChain);
				return array_list.$new();
			};
		} catch (err) {
			console.log('[-] GMS TrustManagerImpl (Android > 7) checkTrustedRecursive {1} check not found');
			//console.log(err);
		}
		try {
			// Bypass GMS TrustManagerImpl.checkTrustedRecursive (Android > 7) {2}
			var array_list = Java.use("java.util.ArrayList");
			var GMS_TrustManagerImpl_Activity_2 = Java.use('com.google.android.gms.org.conscrypt.TrustManagerImpl');
			GMS_TrustManagerImpl_Activity_2.checkTrustedRecursive.implementation = function(certs, host, clientAuth, untrustedChain, trustAnchorChain, used) {
				console.log('[+] Bypassing GMS TrustManagerImpl (Android > 7) checkTrustedRecursive {2} check: '+ host + ' ' + trustAnchorChain + ' '+ untrustedChain);
				return array_list.$new();
			};
		} catch (err) {
			console.log('[-] GMS TrustManagerImpl (Android > 7) checkTrustedRecursive {2} check not found');
			//console.log(err);
		}
		try {
			// Bypass GMS TrustManagerImpl.verifyChain (Android > 7) {1} (probably no more necessary)
			var GMS_TrustManagerImpl_Activity_3 = Java.use('com.google.android.gms.org.conscrypt.TrustManagerImpl');
			GMS_TrustManagerImpl_Activity_3.verifyChain.implementation = function(untrustedChain, trustAnchorChain, host, clientAuth, ocspData, tlsSctData) {
				console.log('[+] Bypassing GMS TrustManagerImpl (Android > 7) verifyChain {1} check: ' + host);
				return untrustedChain;
			};   
		} catch (err) {
			console.log('[-] GMS TrustManagerImpl (Android > 7) verifyChain {1} check not found');
			//console.log(err);
		}
		try {
			// Bypass GMS TrustManagerImpl.verifyChain (Android > 7) {2} (probably no more necessary)
			var GMS_TrustManagerImpl_Activity_4 = Java.use('com.google.android.gms.org.conscrypt.TrustManagerImpl');
			GMS_TrustManagerImpl_Activity_4.verifyChain.implementation = function(untrustedChain, trustAnchorChain, host, clientAuth) {
				console.log('[+] Bypassing GMS TrustManagerImpl (Android > 7) verifyChain {2} check: ' + host);
				return untrustedChain;
			};   
		} catch (err) {
			console.log('[-] GMS TrustManagerImpl (Android > 7) verifyChain {2} check not found');
			//console.log(err);
		}
